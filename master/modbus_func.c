...
void Read_Holding_Reg(unsigned char *loc_ptrmas_input, unsigned char *loc_ptrmas_output) //x03
{
	...
	*(loc_ptrmas_output + 2) = *(loc_ptrmas_input + 5)*2;
	global_count++;
	
	//Значение регистра модбас
	adr = *(loc_ptrmas_input + 3);
	f = *(loc_ptrmas_input + 5);
...
void Write_single_register(unsigned char *loc_ptrmas_input, unsigned char *loc_ptrmas_output) //x06
{
	...
	*(loc_ptrmas_output + 2) = *(loc_ptrmas_input + 2);
	global_count++;
	*(loc_ptrmas_output + 3) = *(loc_ptrmas_input + 3);
	global_count++;
	
	//Значение регистра modbus
	*(loc_ptrmas_output + 4) = *(loc_ptrmas_input + 4);
	global_count++;
	*(loc_ptrmas_output + 5) = *(loc_ptrmas_input + 5);
	global_count++;
